...
from app.services.auth_service import AuthService
from app.api.dependencies import get_current_user, security
from app.repositories.user_repository import UserRepository  # NEW: Add this import
...
@router.post("/logout", status_code=status.HTTP_200_OK)
def logout(
    current_user: User = Depends(get_current_user),  # Changed to get authenticated user
    db: Session = Depends(get_db)
):
    """
    Logout the current user by clearing their active session
    
    Requires valid JWT token in Authorization header
    """
    user_repo = UserRepository(db)
    user_repo.clear_active_token(current_user.user_id)
